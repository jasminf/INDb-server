...
class MarvelApi {
  ...
  fetchComicBookByTitle(comicBookTitle) {
    return this.client.get(`https://gateway.marvel.com/v1/public/comics?title=${comicBookTitle}&limit=15&ts=1&apikey=${marvelApiKey}`)
      .then((response) => {
        return this.handleResponse(response).data.results;
      })
      .catch((error) => {
